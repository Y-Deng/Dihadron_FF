...
inline int test_1D_BSpline_interpolate()
{
    ...
    vector<vector<double>> m_sample_x = {linspace(lower + 1.e-4, lower + 1.9e-4, degree),
                                         linspace(lower + 2.e-4, upper - 2.e-4, N - 2 * degree),
                                         linspace(upper - 1.9e-4, upper - 1.e-4, degree)};

    const std::vector<double> sample_x = linspace(lower + 1.e-3, upper - 1.e-3, N);

    cout << "sample_x: " << sample_x.size() << endl;
    ...
    cout << "finished interpolate" << endl;
    ...
    vector<double> test_x = linspace(lower, upper, 100);
    ...
    BSplineInterpolate_1D interpolator2;
    interpolator2.load("./coefficients1D.txt");
...
inline int test_2D_BSpline_interpolate()
{
    ...
    // Read the interpolated coefficients from a file
    BSplineInterpolate_2D interpolator2;
    interpolator2.load("./coefficients2D.txt");
...
inline int test_3D_BSpline_interpolate()
{
    const int x_degree = 2;
    const int y_degree = 2;
    const int z_degree = 2;
    ...
    const vector<vector<vector<double>>> sample_f = [&]() {
        vector<vector<vector<double>>> f(sample_x.size(),
                                         vector<vector<double>>(sample_y.size(), vector<double>(sample_z.size())));
        for (size_t i = 0; i < sample_x.size(); ++i)
        {
            for (size_t j = 0; j < sample_y.size(); ++j)
            {
    ...
    auto start = std::chrono::high_resolution_clock::now();
    BSplineInterpolate_3D interpolator(x_degree, y_degree, z_degree, lower, upper, lower, upper, lower, upper, sample_x,
