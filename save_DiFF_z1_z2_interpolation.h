...
inline int save_DiFF_z1_z2_interpolation(string filename)
{
    ...
    vector< double > interpolation_z_points = linspace(interpolation_z_lower+1.e-5, interpolation_z_upper-1.e-5, interpolation_z_num_points);
    ...
    for(int i_flavor = 0; i_flavor < flavor_ids.size(); ++i_flavor)
    {
        ...
        vector< vector< double > > interpolation_DiFF_z1_z2_points = [&interpolation_z_num_points, &interpolation_z_points, &id](){
            vector< vector< double > > result(interpolation_z_num_points, vector<double>(interpolation_z_num_points));
            for(int i = 0; i < interpolation_z_num_points; ++i)
            {
                for(int j = 0; j < interpolation_z_num_points; ++j)
                {
                    ...
                    double DiFF_z1_z2 = FF_dihadron_plus_minus_FFs(z1, z2, id);
        ...
        BSplineInterpolate_2D interpolation_DiFF_z1_z2(
            ...
            interpolation_z_lower, interpolation_z_upper, interpolation_z_lower, interpolation_z_upper, 
            interpolation_z_points, interpolation_z_points, interpolation_DiFF_z1_z2_points
        );
        ...
        TGraph* g = new TGraph(interpolation_z_num_points, &interpolation_z_points[0], &interpolation_DiFF_z1_z2_points[check_index][0]);
        ...
        for(int i = 0; i < interpolation_z_num_points; ++i)
        {
            test_DiFF[i] = interpolation_DiFF_z1_z2(interpolation_z_points[check_index], interpolation_z_points[i]);
        }

        TGraph* g_test = new TGraph(interpolation_z_num_points, &interpolation_z_points[0], &test_DiFF[0]);
