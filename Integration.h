...
template <typename T>
using integrand_function = std::function<double (double eta, T par)>;
...
)
{
    ...
    try{
        ...
        {
            std::cerr <<"Integration failed " <<"Integration result: " << result << " ± " << error << std::endl;
            throw GSLIntegrationError(
                std::string("GSL Integration Error: ") + gsl_strerror(status)
            );
        }
    }
